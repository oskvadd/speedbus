...
void    send(char * data, int len){
	...
	usleep(2500);
      ...
      // Important to use +diff because the data wont be at the same place if the adress is twice as longe...
      if((unsigned char)data[5+diff] == 0x01) // If the package are going to recive a response, set got_resp to zero, else, leave it as it was.
      got_resp = 0;

      if((unsigned char)data[0] == 0xff && (unsigned char)data[1] == 0xff) // Dont wait for response on broadcast
	break;
      if((unsigned char)data[6+diff] == 0) // Dont wait for response on response
	break;
      if((unsigned char)data[5+diff] != 0x01) // Dont wait for response when using the nonresponse protocoll
	break;
