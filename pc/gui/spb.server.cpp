...
bool add_user(config_t * server_cfg, char *user, char *pass,
	      bool is_admin_s);
bool del_user(config_t * server_cfg, char *user);
bool mod_user(config_t * server_cfg, char *user, char *user_n,
	      char *pass_n, bool is_admin_s);
	...
	for (int i = 0; i < serial_p->backe->devids; i++) {
		...
		if (serial_p->backe->daddr1[i] == p_data[2] && serial_p->backe->daddr2[i] == p_data[3]) {	// Device id is found in the MAIN device list
			...
			for (int ii = 0; ii < MAX_EVENT; ii++) {
				...
				if ((serial_p->backe->event_exist[ii])
				    && strcmp(serial_p->backe->
					      event_exec[i][ii],
					      "getvars") == 0) {
					int count =
					    (unsigned char) serial_p->
					    backe->event_data[i][ii][0];
	...
	if (file != NULL) {
		...
		while (fgets(line, sizeof line, file) != NULL) {	/* read a line */
			...
			if (sscanf(line, "%d %s\n", &devid, addr) == 2) {
				...
				device_add((void *) NULL, addr1, addr2,
					   devid);
				serial_p->device_id[serial_p->device_num] =
				    devid;
				strcpy(serial_p->
				       device_addr[serial_p->device_num],
				       addr);
				serial_p->device_num++;
			}
		}
...
void device_add(print_seri * serial_p, char addr1, char addr2, int devid)
{
	...
	//
	char name[7];
	sprintf(name, "%d.%d", (unsigned char) addr1,
		(unsigned char) addr2);
	if (debug)
		std::cerr << "\n" << serial_p->
		    device_num << ":" << name << "\n";
		...
		if (!file_exists(serial_p->ctty)) {
			...
			for (int i = 0; i < MAX_LISTEN; i++) {
				if (serial_p->server->session_open[i]) {
					serial_p->server->send_data(i,
								    "info Serial Interface Down!\n",
								    strlen
								    ("info Serial Interface Down!\n"));
				}
			}
			...
			while (1) {
				...
				if (serial_port.IsOpen()) {
					...
					printf("Serial port %s is open!\n",
					       serial_p->ctty);
					for (int i = 0; i < MAX_LISTEN;
					     i++) {
						if (serial_p->server->
						    session_open[i]) {
							serial_p->server->
							    send_data(i,
								      "info Serial Interface Up Again!\n",
								      strlen
								      ("info Serial Interface Up Again!\n"));
						}
					}
...
bool spb_inalize_notify(print_seri * serial_p)
{
	...
	if (file) {
		...
		while (write_c <= read_counter && write_c < 10
		       && line_c > -1) {
			strncpy(serial_p->notify[line_c],
				tmp_notify[write_c], MAX_NOTIFY_SIZE + 50);
			write_c++;
			line_c--;
		}
	} else {
...
bool spb_write_notify(print_seri * serial_p, const char *w_log, int prio)
{
	...
	for (int i = MAX_NOTIFY_STACK - 2; i >= 0; i--) {
		strncpy(serial_p->notify[i + 1], serial_p->notify[i],
			MAX_NOTIFY_SIZE + 50);
	}
	sprintf(serial_p->notify[0], "%d %d %d %s", (int) time(0),
		serial_p->notify_nr, prio, w_log);
	FILE *file;
	file = fopen(SERVER_CONFIG_DIR SERVER_NOTIFY_FILE, "a+");
	fprintf(file, "%d %d %d %s\n", (int) time(0), serial_p->notify_nr,
		prio, w_log);
	fclose(file);
	...
	for (int i = 0; i < MAX_LISTEN; i++) {
		if (serial_p->server->session_open[i]) {
			serial_p->server->send_data(i, tmp_send,
						    strlen(tmp_send));
		}
	}
	...
	if (!serial_p->server->session_open[listnum]) {
		...
		for (int i = 0; i < userc; i++) {
			...
			if (strncmp(data, tmp, len) == 0) {
				...
				sprintf(tmp, "notifc %d\n",
					serial_p->notify_nr);
				serial_p->server->send_data(listnum, tmp,
							    strlen(tmp));
				...
				serial_p->server->session_open[listnum] =
				    1;
				wtime();
				printf
				    ("User: (%s) logged in from IP: (%s)\n",
				     user,
				     serial_p->server->client_ip[listnum]);

				return 1;
			}
		}
		err =
		    serial_p->server->send_data(listnum, "Login Failed\n",
						strlen("Login Failed\n"));
		...
		if (strncmp(data, "adduser", 7) == 0) {
			...
			sscanf(data, "adduser %s %s %d\n", user, pass,
			       &is_admin);
			if (add_user
			    (serial_p->server_cfg, user, pass, is_admin)) {
				serial_p->server->send_data(listnum,
							    "sinfo User added!\n",
							    strlen
							    ("sinfo User added!\n"));
		...
		if (strncmp(data, "moduser", 7) == 0) {
			...
			if (mod_user
			    (serial_p->server_cfg, moduser, user, pass,
			     is_admin)) {
				serial_p->server->send_data(listnum,
							    "sinfo User modded!\n",
							    strlen
		...
		if (strncmp(data, "get-tty", 7) == 0) {
			...
			if (config_lookup_string
			    (serial_p->server_cfg, "tty", &tty)) {
				if (strlen(tty) < 95) {
					sprintf(ctty, "ctty %s\n", tty);
					serial_p->server->
					    send_data(listnum, ctty,
						      strlen(ctty));
				}
			}
			char path[100];
			...
			while (fgets(ttyt, sizeof(ttyt), pipe) > 0) {
				...
				*nlptr = '\0';
				...
				serial_p->server->send_data(listnum, send,
							    strlen(send));
			}
		}
		...
		if (strncmp(data, "deveid ", 7) == 0) {
			...
			if (devid > 0) {
				...
				char openf[50];
				sprintf(openf,
					SERVER_CONFIG_DIR "devs/%d.spb",
					devid);
				dfile = fopen(openf, "rb");
				if (dfile == NULL) {
					serial_p->server->
					    send_data(listnum,
						      "deveinfo File does not exist\0",
						      strlen
						      ("deveinfo File does not exist\0"));
					return 0;
				}
				...
				while (i > 0) {
					tbuffer[i] = 0;
					sprintf(sbuffer, "devei %s",
						tbuffer);
					serial_p->server->
					    send_data(listnum, sbuffer,
						      strlen(sbuffer));
					usleep(50000);
					i = fread(tbuffer, 1, 990, dfile);
		...
		if (strncmp(data, "devec ", 6) == 0) {
			...
			FILE *clear_file;
			char cfile[200];
			sprintf(cfile, SERVER_CONFIG_DIR "devs/%d.spb",
				devid);
		...
		if (strncmp(data, "devei ", 6) == 0) {
			...
			char afile[200];
			char nr[20];
			sprintf(afile, SERVER_CONFIG_DIR "devs/%d.spb",
				devid);
			a_file = fopen(afile, "a+");
			sprintf(nr, "%d", devid);
			int devid_len = strlen(nr);
			fwrite(data + 7 + devid_len, (len - 8) - devid_len,
			       1, a_file);
			fclose(a_file);
		}
...
{
	if (FD_ISSET
	    (serial_p->server->listen_sock, &serial_p->server->socks)) {
...
void run(print_seri * serial_p)
{
	...
	while (1) {
		...
		sock =
		    select(serial_p->server->highsock + 1,
			   &serial_p->server->socks, NULL, NULL, NULL);
		if (sock > 0)
			runselect(serial_p);
		if (sock < 0)
	      ...
	      bool is_admin_s)
{
	...
	if (!users_c) {
		...
		users_c =
		    config_setting_add(root, "users", CONFIG_TYPE_LIST);
	}
	...
	if (!config_write_file(server_cfg, SERVER_CONFIG_URI)) {
		printf
		    ("Unable to write server.cfg :/ do i have the right access? Bye!\n");
	}
		...
		for (int i = 0; i < count; ++i) {
			...
			if (config_setting_lookup_string
				...
				if (strcmp(user, user_c) == 0) {
					config_setting_remove_elem(setting,
								   i);
					status++;
					break;
				}
	...
	if (setting != NULL) {
		...
		for (int i = 0; i < count; ++i) {
			...
			if (config_setting_lookup_string
				...
				if (strcmp(user, user_c) == 0) {
					if (strlen(user_n) > 0) {
						strncpy(user_s, user_n,
							MAX_LOGIN_TEXT);
						strncpy(users[index][1],
							user_s,
							MAX_LOGIN_TEXT);
						member =
					...
					if (strlen(pass_n) > 0) {
						strncpy(passwd, pass_n,
							MAX_LOGIN_TEXT);
						md5sum(passwd);
						strncpy(users[index][2],
							passwd,
							MAX_LOGIN_TEXT);
						member =
						    config_setting_get_member
					...
					config_setting_set_bool(member,
								is_admin_s);
					goto del_user_end;
				}
			}
...
bool set_tty(print_seri * serial_p, const char *tty)
{
	if (tty != NULL && strlen(tty) > 0) {
		...
		if (!setting) {
			setting =
			    config_root_setting(serial_p->server_cfg);
			setting =
			    config_setting_add(setting, "tty",
					       CONFIG_TYPE_STRING);
			if (!setting)
				return 0;
		}
		...
		if (!config_write_file
		    (serial_p->server_cfg, SERVER_CONFIG_URI)) {
...
void make_new_admin(config_t * server_cfg,
		    char user_arg[2][MAX_LOGIN_TEXT])
{
	...
	if (1) {
		...
		if (!config_read_file(&server_cfg, SERVER_CONFIG_URI)) {
			...
			if (fgets(tmp, 50, pipe) == NULL) {
				...
				printf("Line %d: %s\n",
				       config_error_line(&server_cfg),
				       config_error_text(&server_cfg));
			...
			if (setting != NULL) {
				...
				for (int i = 0; i < count; ++i) {
					...
					if ((config_setting_lookup_string
						...
						strncpy(users[userc][1],
							user,
							MAX_LOGIN_TEXT);
						strncpy(users[userc][2],
							pass,
							MAX_LOGIN_TEXT);
						is_admin[userc] =
						    is_admin_a;
		...
		if (argc > 2) {
			...
			if (strcmp(argv[1], "moduser") == 0) {
				mod_user(&server_cfg, argv[2], argv[3],
					 argv[4], atoi(argv[5]));
			}
		...
		pthread_t printr;
		pthread_create(&printr, NULL, &print_ser_backend,
			       (void *) &serial_p);
		addr1 = addr2 = 20;
		...
		if (serial_port.IsOpen()) {
			if (serial_p.device_num < 1) {
				...
				int len = 8;
				char getdevs[20] =
				    { 0xFF, 0xFF, addr1, addr2, 0x03, 0x01,
			     0x01, 0x00 };
				for (int i = 0; i < 8; i++) {
					if (i == 0 || i == 4)
						send(getdevs, len);
