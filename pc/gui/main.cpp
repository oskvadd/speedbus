...
void m_send(void *ptr, char *data, int len)
{
	...
	if (rdata->remote) {
		if (!((ProgressData *) rdata->share)->connected) {
			gtk_label_set_text(GTK_LABEL(rdata->label),
					   "ERROR: Not connected");
			return;
		}
...
void exec_package(void *ptr, char *data, int counter)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) ptr;
	int devid = 0;
	...
	if (debug) {
		...
		case 0:
			std::
			    ...
			    std::
			    dec << (data[2] & 0xff) << "." << (data[3] &
							       0xff) <<
			    "' to '" << std::
			    dec << (data[0] & 0xff) << "." << (data[1] &
							       0xff) <<
			    "'";
			break;
		case 1:
			std::
			    cerr <<
			    "Recived device list aknowlegde (0x01) from '"
			    << std::
			    dec << (data[2] & 0xff) << "." << (data[3] &
							       0xff) <<
			    "'";
		...
		case 3:
			std::
			    cerr << "Recived config fetch (0x03) from '" <<
			    std::
			    dec << (data[2] & 0xff) << "." << (data[3] &
							       0xff) <<
			    "'";
		...
		case 170:
			wtime();
			std::cerr << "Recived ALARM (0xAA) from '" << std::
			    dec << (data[2] & 0xff) << "." << (data[3] &
							       0xff) <<
		...
		default:
			std::
			    cerr << "SUCESS: Recived package from: '" <<
			    std::dec << (static_cast <
					 int >(data[2]) & 0xff) <<"." <<
			    (static_cast <
			     int >(data[3]) & 0xff) <<"' With data: '";
		}
		for (int i = 0; i < counter - 9; i++) {
			if (debug)
				std::cerr << std::hex << (static_cast <
							  int
							  >(data[i + 6]) &
							  0xff);
			if (i < counter - 10) {
	...
	if ((unsigned char) data[0] == addr1
	...
	} else if ((unsigned char) data[0] == 0xFF
		...
		case 3:
			...
			case 1:
				devid = 0;
				for (int i = 0; i < counter - 11; i++) {	// Run this backwards to get the bytes right                                                                                      
					devid <<= 8;
					devid +=
				...
				sprintf(print,
					"Recived IAH from: %d.%d (%d)",
					(unsigned char) data[2],
					(unsigned char) data[3], devid);
				gtk_label_set_text(GTK_LABEL(rdata->label),
						   print);
				break;
			}
		...
		if (len > 0) {
			...
			if (strncmp(data, "userlist ", 9) == 0) {
				...
				sscanf(data, "userlist %s %d\n", username,
				       &is_admin);
				gtk_list_store_append(rdata->
						      rserv_store_user,
						      &rdata->rserv_iter);
				gtk_list_store_set(rdata->rserv_store_user,
						   &rdata->rserv_iter,
						   COL_NAME, username,
						   COL_AGE, is_admin, -1);
				rdata->rserv_model_user =
				    GTK_TREE_MODEL(rdata->
						   rserv_store_user);
			...
			if (strncmp(data, "ttylist ", 8) == 0) {
				...
				gtk_list_store_append(rdata->
						      rserv_store_tty,
						      &rdata->rserv_iter);
				gtk_list_store_set(rdata->rserv_store_tty,
						   &rdata->rserv_iter,
						   COL_NAME, tty, -1);
				rdata->rserv_model_tty =
				    GTK_TREE_MODEL(rdata->rserv_store_tty);
				gtk_tree_view_set_model(GTK_TREE_VIEW
							(rdata->
							 rserv_tty_box),
							rdata->
			...
			if (strncmp(data, "devec ", 6) == 0) {
				gtk_text_buffer_set_text(rdata->
							 rdeve_text_buffer,
							 "\0\0", -1);
			}
			if (strncmp(data, "devei ", 6) == 0) {
				gtk_text_buffer_get_end_iter(rdata->
							     rdeve_text_buffer,
							     &rdata->
							     rdeve_text_iter);
				gtk_text_buffer_insert(rdata->
						       rdeve_text_buffer,
						       &rdata->
						       rdeve_text_iter,
						       data + 6,
			...
			if (strncmp(data, "notifya ", 8) == 0) {
				...
				if (len < 210
				    && sscanf(data, "notifya %d %d %d",
					      &date, &id, &priorty) == 3) {
					// Take care of the notify message number
					sprintf(msg, "notifya %d %d %d",
						date, id, priorty);
					int tmp_len = strlen(msg);
					memset(msg, 0, 200);
					if (tmp_len < 15)
						return 0;
					strncpy(msg, data + tmp_len,
						len - tmp_len);
					char *tmp = strchr(msg, '\n');
					*tmp = '\0';
					struct tm *timeinfo =
			...
			printf("Connection seems to have died %d :/\n",
			       len);
			gtk_widget_show(((ProgressData *) rdata->share)->
					window);
			gtk_label_set_text(GTK_LABEL
					   (((ProgressData *) rdata->
					     share)->label1),
		...
		while (serial_port.rdbuf()->in_avail() > 0) {
			...
			if (next_byte == 0x7e && e == 1) {
				...
				if (crcstrc(data, counter) && counter > 5) {	// Min packet length 5
					if ((unsigned char) data[0] == addr1 && (unsigned char) data[1] == addr2 && ((unsigned char) data[5] == 1 && (unsigned char) data[6] != 0)) {	// IMPORTANT to check that the 
			...
			if (counter > 99) {
				...
				if (debug) {
					std::
					    cerr <<
					    "Killed package longer than 99bytes"
					    << std::endl;
				}
			}
			if (debug)
...
static void tty_to_combo(GtkWidget * combo)
{
	GList *glist = NULL;
...
static bool open_tty_button(GtkWidget * widget, gpointer data)
{
	addr1 = 20;
	addr2 = 20;
	...
	string =
	    (char *)
	    gtk_entry_get_text(GTK_ENTRY(GTK_COMBO(pdata->combo)->entry));
...
static void speedbus_fill_devlist(gpointer data)
{
	...
	for (int i = 0; i < device_num; i++) {
		...
		gtk_list_store_set(store, &iter,
				   COL_NAME, device_id[i],
				   COL_AGE, device_addr[i], -1);
	}
...
static GtkWidget *new_scan_list(void)
{
...
static gboolean speedbus_pbar_scanning_pack_send(gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	int len = 8;
	char getdevs[20] =
	    { 0xFF, 0xFF, addr1, addr2, 0x03, 0x01, 0x01, 0x00 };
...
static gboolean speedbus_pbar_scanning(gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	gdouble new_val;
	char hej[50];
	...
	new_val =
	    gtk_progress_bar_get_fraction(GTK_PROGRESS_BAR(rdata->pbar)) +
	    0.013;
	if (new_val > 1.0) {
		sprintf(hej, "100%%");
		gtk_progress_bar_set_text(GTK_PROGRESS_BAR(rdata->pbar),
					  hej);
		rdata->scan_lock = 0;
		sprintf(hej, "Scanning done, found %d Devices",
			device_num);
	...
	gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(rdata->pbar),
				      new_val);
	return 1;
}


static void speedbus_unit_scan(GtkWidget * some, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	if (!rdata->scan_lock) {
		gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR
					      (rdata->pbar), 0);
		rdata->timer =
		    g_timeout_add(100, speedbus_pbar_scanning, rdata);
		rdata->scan_lock = 1;
		rdata->scan_counter = 0;
		speedbus_pbar_scanning_pack_send(rdata);
		g_timeout_add(4000, speedbus_pbar_scanning_pack_send,
			      rdata);
	}
...
static int get_variable_gui(void *data, int var_id, bool from_gui)
{
	...
	int devids = 0;
	for (int i = 0; i < device_num; i++) {
		if (device_id[i] == rdata->c_devid) {	// Device id is found in the MAIN device list
			devids = i;
			break;
		}
	}
	...
	if (from_gui) {
		if (strcmp
		    (rdata->spb_widget_variable_type[var_id],
		     "scale") == 0) {
			return (char)
			    ...
			    (gtk_range_get_adjustment
			     (GTK_RANGE
			      (rdata->
			       spb_widgets[rdata->
					   spb_widget_variable_source
					   [var_id]])));
		}
	} else {
		return backend_exec_ops(rdata->backe, devids, var_id + 257,
					rdata->
					spb_widget_vars_data[var_id], 0);
	}
}

static char set_variable_gui(void *data, int var_id, int v_data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	var_id -= 257;
	...
	if (strcmp(rdata->spb_widget_variable_type[var_id], "label") == 0) {
		sprintf(print, rdata->spb_widget_vars_printf[widget_id],
			...
			get_variable_gui(rdata,
					 rdata->
					 spb_widget_vars[widget_id][3] +
					 257, 0));
		gtk_label_set_text(GTK_LABEL
				   (rdata->spb_widgets[widget_id]), print);
	}
	if (strcmp(rdata->spb_widget_variable_type[var_id], "scale") == 0) {
		...
		adj =
		    gtk_range_get_adjustment(GTK_RANGE
					     (rdata->
					      spb_widgets[widget_id]));
		gtk_adjustment_set_value(adj,
					 get_variable_gui(rdata,
							  var_id + 257,
							  0));
		gtk_range_set_adjustment(GTK_RANGE
					 (rdata->spb_widgets[widget_id]),
					 adj);
	}
	...
	for (int i = 0; i < device_num; i++) {
		if (device_id[i] == rdata->c_devid) {	// Device id is found in the MAIN device list
			for (int ii = 0; ii < MAX_EVENT; ii++) {
				if ((rdata->backe->event_exist[ii])
				    && strcmp(rdata->backe->
					      event_exec[i][ii],
					      "getvars") == 0) {
					int count =
					    (unsigned char) rdata->backe->
					    event_data[i][ii][0];
					for (int iii = 0; iii < count;
					     iii++) {
						if ((unsigned char) rdata->
						    backe->
						    event_data[i][ii][1 +
								      iii]
						    ==
...
static void spb_device_event(GtkWidget * some, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	for (int i = 0; i < rdata->spb_widget_c; i++) {
		if (rdata->spb_widgets[i] == some) {
			...
			backend_exec(rdata->backe,
				     rdata->spb_widget_event[i],
				     rdata->c_devid, 1);
...
static void load_device(gpointer data, int devid)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	config_t cfg;
	...
	gtk_box_pack_start(GTK_BOX(rdata->box2), rdata->box3, FALSE, FALSE,
			   source_width);
	...
	gtk_box_pack_start(GTK_BOX(rdata->box3), rdata->label1, FALSE,
			   FALSE, 0);
	...
	if (setting != NULL) {
		...
		for (int i = 0; i < count; ++i) {
			...
			if (config_setting_lookup_int(book, "row", &row)
			    || rdata->spb_widget_row_c == 0) {
				for (; rdata->spb_widget_row_c <= row;
				     rdata->spb_widget_row_c++) {
					rdata->spb_widget_row[rdata->
							      spb_widget_row_c]
			...
			if (strcmp(type, "scale") == 0) {
				...
				if ((config_setting_lookup_int
					...
					rdata->
					    spb_widget_variable_source
					    [var_id] = rdata->spb_widget_c;
					strcpy(rdata->
					       spb_widget_variable_type
					       [var_id], "scale");
				}
	...
	if (setting != NULL) {
		...
		for (int i = 0; i < count; ++i) {
			...
			if (!
			    ...
			    ||
			    !(config_setting_lookup_int
			      (book, "runon_open", &runon_open)))
				continue;
			if (sizeof(event) > MAX_EVENT
			    || sizeof(type) > MAX_BUFFER)
				continue;
			if (runon_open)
				backend_exec(rdata->backe, event,
					     rdata->c_devid, 1);
		}
	}
			 ...
			 GtkTreeViewColumn * col, gpointer data)
{
	...
	if (gtk_tree_model_get_iter(model, &iter, path)) {
		int devid;
		gtk_tree_model_get(model, &iter, 0, (gpointer) & devid,
				   -1);
		const char *addr;
...
gboolean rdev_stamp(GtkWidget * some, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	const char *getcaddr1 =
	    gtk_entry_get_text(GTK_ENTRY(rdata->rdev_addr_tbox1));
	const char *getcaddr2 =
	    gtk_entry_get_text(GTK_ENTRY(rdata->rdev_addr_tbox2));
	int getaddr1 = atoi(getcaddr1);
	int getaddr2 = atoi(getcaddr2);
	if ((getaddr1 == 0 && getcaddr1[0] != '0')
	    || (getaddr2 == 0 && getcaddr2[0] != '0') || getaddr1 > 255
	    || getaddr2 < 0 || getaddr2 > 255 || getaddr2 < 0) {
		gtk_label_set_text(GTK_LABEL(rdata->rdev_stamp_label),
				   "That does not look like a vaild adress!");
		return 1;
	}
	char tmp[100];
	...
	for (int i = 0; i <= 5; i++) {
		if (i > 5) {
			sprintf(tmp, "No response on stamp...", getaddr1,
				getaddr2);
			gtk_label_set_text(GTK_LABEL
					   (rdata->rdev_stamp_label), tmp);
			break;
		}
		int len = 11;
		char getdevs[50] =
		    { rdata->addr1, rdata->addr2, addr1, addr2, 0x03, 0x00,
	     0x03, 0x01, getaddr1, getaddr2, 0x00 };
		m_send(rdata, getdevs, len);
		usleep(500000);
		if (got_resp) {
			sprintf(tmp, "Succefully stamped %d.%d...",
				getaddr1, getaddr2);
			gtk_label_set_text(GTK_LABEL
					   (rdata->rdev_stamp_label), tmp);
			...
			int len2 = 7;
			char getdevs2[50] =
			    { getaddr1, getaddr2, addr1, addr2, 0x03, 0x00,
		    0x01, 0x00 };
		...
		device_add(rdata, rdata->addr1, rdata->addr2,
			   rdata->c_devid);
...
gboolean rdev_gui_destroy(GtkWidget * some, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
...
gboolean rdev_gui_delete(GtkWidget * some, GdkEvent * event, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	gtk_widget_set_sensitive(rdata->devbutton, true);
	gtk_widget_destroy(some);
...
void rserver_settings_users_box(GtkTreeView * treeview, GtkTreePath * path,
				GtkTreeViewColumn * col, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	GtkTreeModel *model;
	...
	if (gtk_tree_model_get_iter(model, &iter, path)) {
		const char *username;
		gtk_tree_model_get(model, &iter, 0, (gpointer) & username,
				   -1);
		int is_admin;
		gtk_tree_model_get(model, &iter, 1, (gpointer) & is_admin,
				   -1);
		// Get addr...
		gtk_entry_set_text(GTK_ENTRY
				   (rdata->rserv_entry_moduser_user),
				   username);
		if (is_admin)
...
void rserver_settings_moduser(GtkWidget * some, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	rdata->rserv_your_clear_counter = 0;
	if (!((ProgressData *) rdata->share)->connected) {
		gtk_label_set_text(GTK_LABEL(rdata->rserv_status_label),
				   "ERROR: Not connected");
		return;
	} else {
		rdata->rserv_store_user =
		    gtk_list_store_new(NUM_COLS, G_TYPE_STRING,
				       G_TYPE_UINT);
		...
		usr =
		    (char *)
		    gtk_entry_get_text(GTK_ENTRY(rdata->rserv_entry_user));
		if (strlen(usr) < 1) {
		...
		pwd =
		    (char *)
		    gtk_entry_get_text(GTK_ENTRY(rdata->rserv_entry_pass));
		pwd_c =
		    (char *)
		    gtk_entry_get_text(GTK_ENTRY
				       (rdata->rserv_entry_pass_c));
		is_admin =
		    gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON
						 (rdata->rserv_is_admin));
		if (strcmp(pwd, pwd_c) == 0) {
			...
			sprintf(send_data,
				"moduser %s \t %d \t %s \t %s\n", modusr,
				is_admin, usr, pwd);
			rdata->sslc.send_data(send_data,
					      strlen(send_data));
		} else {
			gtk_label_set_text(GTK_LABEL
					   (rdata->rserv_status_label),
					   "Passwords does not match");
		}
	}
}

void rserver_settings_adduser(GtkWidget * some, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	rdata->rserv_your_clear_counter = 0;
	if (!((ProgressData *) rdata->share)->connected) {
...
void rserver_settings_deluser(GtkWidget * some, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	rdata->rserv_your_clear_counter = 0;
	if (!((ProgressData *) rdata->share)->connected) {
...
void rserver_settings_tty_list(GtkWidget * some, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	rdata->rserv_your_clear_counter = 0;
	if (!((ProgressData *) rdata->share)->connected) {
		...
		rdata->rserv_store_tty =
		    gtk_list_store_new(1, G_TYPE_STRING);
		char *pattern;
		pattern =
		    (char *)
		    gtk_entry_get_text(GTK_ENTRY(rdata->rserv_tty_entry));
		char send_data[RECV_MAX];
...
void rserver_settings_tty_open(GtkWidget * some, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	rdata->rserv_your_clear_counter = 0;
	if (!((ProgressData *) rdata->share)->connected) {
...
void rserver_settings_tty_close(GtkWidget * some, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	rdata->rserv_your_clear_counter = 0;
	if (!((ProgressData *) rdata->share)->connected) {
...
gboolean rserver_settings_destroy(GtkWidget * some, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
...
gboolean rserver_settings_delete(GtkWidget * some, GdkEvent * event,
				 gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	g_source_remove(rdata->rserv_your_clear_timeout);
...
static gboolean rserver_settings_your_clear(gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	char *usr;
	usr =
	    (char *)
	    gtk_entry_get_text(GTK_ENTRY(rdata->rserv_entry_your_pass));
	...
	if (rdata->rserv_your_clear_counter > 119) {
		...
		rdata->rserv_your_clear_counter = 0;
		gtk_label_set_text(GTK_LABEL(rdata->rserv_status_label),
				   "Your password cleared due to inactivity(120sec)");
	}
...
gboolean rdeve_load_devid(GtkWidget * buffer, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	const char *devid =
	    gtk_entry_get_text(GTK_ENTRY(rdata->rdeve_devid_entry));
	char send_str[strlen(devid) + 10];
	if (atoi(devid) < 0) {
		gtk_statusbar_push(GTK_STATUSBAR(rdata->rdeve_status_bar),
				   0, "Check devid number, error...");
		return 1;
	}
...
gboolean rdeve_update_statusbar(GtkTextBuffer * buffer, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
...
gboolean rdeve_mark_set_callback(GtkTextBuffer * buffer,
...
bool rdeve_fetch_config(gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	...
	char *ctext =
	    gtk_text_buffer_get_text(rdata->rdeve_text_buffer, &start,
				     &end, FALSE);
	...
	if (p == NULL) {
		gtk_statusbar_push(GTK_STATUSBAR(rdata->rdeve_status_bar),
				   0,
				   "Error opening .tmp.conf premissions?");
		return 0;
	}
...
bool rdeve_open_config(gpointer data, config_t * cfg)
{
...
gboolean rdeve_config_upload(GtkWidget * buffer, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	...
	while (i > 0) {
		tbuffer[i + 1] = '\0';
		sprintf(sbuffer, "devei %d %s", rdata->rdeve_event_number,
			tbuffer);
...
gboolean rdeve_config_check(GtkWidget * buffer, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
...
gboolean rdeve_config_reload(gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	...
	while (fgets(line, 4096, fr) != NULL) {
		gtk_text_buffer_get_end_iter(rdata->rdeve_text_buffer,
					     &rdata->rdeve_text_iter);
		gtk_text_buffer_insert(rdata->rdeve_text_buffer,
				       &rdata->rdeve_text_iter, line,
				       strlen(line));
	}
	fclose(fr);
...
gboolean rdeve_event_change(GtkWidget * buffer, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	rdata->rdevee_load_type_hardcode =
	    gtk_combo_box_get_active(GTK_COMBO_BOX
				     (rdata->rdevee_combo_opt)) + 1;
	rdeve_event_load(buffer, rdata);
}


gboolean rdeve_event_save(GtkWidget * buffer, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;

	config_t cfg;
	if (!rdeve_fetch_config(rdata) || !rdeve_open_config(rdata, &cfg)
	    || !rdata->rdevee_event_loaded)
		return 0;
		...
		for (int i = 0; i < count; ++i) {
			...
			if (event == rdata->rdevee_loaded_nr) {
				if (strcmp(type, "send") == 0) {
					...
					gdata_t =
					    config_setting_get_member(book,
								      "data");
					int edcount =
					    config_setting_length(gdata_t);
					int edata[edcount];
					int ed;
							...
							if (opts_t != NULL) {
								...
								if (opts_c) {
									for (int oi = 0; oi < opts_c; ++oi) {
										...
										int oat, obits;
										bool is_set = 1;
										if (config_setting_lookup_string(opts_e, "name", &oname) && config_setting_lookup_int(opts_e, "at", &oat) && config_setting_lookup_int(opts_e, "bits", &obits)) {
											...
											if (!is_set) {
												...
												int pattern;
												for (int p1 = 0; p1 < 7; p1++) {	// UNSTABLE 121230 , i was not able to test this as much as i really should, so be aware of Bugs that may show up --- START
													if (p1 < bstart) {
														pattern
														    <<=
														    1;
									...
									for (int e_r = 1; e_r < max_bytes; e_r++) {	// not removed, and after that we add new ones
										config_setting_t
										    *c_t;
...
gboolean rdeve_event_load(GtkWidget * buffer, gpointer data)
{
	...
	if (rdata->rdevee_event_loaded) {
		if (!rdata->rdevee_load_type_hardcode) {
			...
			gtk_box_pack_start(GTK_BOX(rdata->rdevee_box3),
					   rdata->rdevee_ebox1, FALSE,
					   TRUE, 0);
		}
		...
		gtk_box_pack_start(GTK_BOX(rdata->rdevee_box3),
				   rdata->rdevee_ebox, FALSE, TRUE, 0);
	...
	sscanf((char *)
	       gtk_entry_get_text(GTK_ENTRY
				  (rdata->rdevee_load_event_entry)), "%d",
	       &levent);
	if (levent < 1) {
		gtk_statusbar_push(GTK_STATUSBAR(rdata->rdevee_status_bar),
				   0, "Cant load, number is fail");
		return 0;
	}
									...
									gtk_statusbar_push
									    (GTK_STATUSBAR
									     (rdata->
									      rdeve_status_bar),
									     0,
									     "data, at, bits do not have the same number of elements");
								}
							}
						}
					}
...
gboolean rdeve_event_edit(GtkWidget * buffer, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	...
	gtk_window_set_position(GTK_WINDOW(rdata->rdevee_gui),
				GTK_WIN_POS_CENTER);
	//gtk_window_set_default_size(GTK_WINDOW(rdata->rdevee_gui), 600, 250);
	g_signal_connect(rdata->rdevee_gui, "delete-event",
			 G_CALLBACK(delete_event), rdata);
	g_signal_connect(rdata->rdevee_gui, "destroy",
			 G_CALLBACK(destroy), rdata);
	...
	gtk_container_add(GTK_CONTAINER(rdata->rdevee_gui),
			  rdata->rdevee_box1);
	rdata->rdevee_load_event_label = gtk_label_new("Event number: ");
	rdata->rdevee_load_event_entry = gtk_entry_new();
	rdata->rdevee_load_event_button =
	    gtk_button_new_with_label("Load event");
	rdata->rdevee_save_event_button =
	    gtk_button_new_with_label("Save event");
	gtk_box_pack_start(GTK_BOX(rdata->rdevee_box2),
			   rdata->rdevee_load_event_label, FALSE, FALSE,
			   0);
	gtk_box_pack_start(GTK_BOX(rdata->rdevee_box2),
			   rdata->rdevee_load_event_entry, FALSE, FALSE,
			   0);
	gtk_box_pack_start(GTK_BOX(rdata->rdevee_box2),
			   rdata->rdevee_load_event_button, FALSE, FALSE,
			   0);
	gtk_box_pack_start(GTK_BOX(rdata->rdevee_box2),
			   rdata->rdevee_save_event_button, FALSE, FALSE,
			   0);
	gtk_box_pack_start(GTK_BOX(rdata->rdevee_box1), rdata->rdevee_box2,
			   FALSE, FALSE, 0);
	g_signal_connect(rdata->rdevee_load_event_button, "clicked",
			 G_CALLBACK(rdeve_event_load), rdata);
	g_signal_connect(rdata->rdevee_save_event_button, "clicked",
			 G_CALLBACK(rdeve_event_save), rdata);


	gtk_box_pack_start(GTK_BOX(rdata->rdevee_box1), rdata->rdevee_box3,
			   FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rdevee_box3),
			   rdata->rdevee_ebox1, FALSE, TRUE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rdevee_box3), rdata->rdevee_ebox,
			   FALSE, FALSE, 0);

	rdata->rdevee_status_bar = gtk_statusbar_new();
	gtk_box_pack_start(GTK_BOX(rdata->rdevee_box1),
			   rdata->rdevee_status_bar, FALSE, FALSE, 0);
	gtk_widget_show_all(rdata->rdevee_gui);
	config_destroy(&cfg);
}


void rdev_editor(GtkWidget * some, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	rdata->rdeve_gui = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_position(GTK_WINDOW(rdata->rdeve_gui),
				GTK_WIN_POS_CENTER);
	//gtk_window_set_default_size(GTK_WINDOW(rdata->rdeve_gui), 600, 750);
	g_signal_connect(rdata->rdeve_gui, "delete-event",
			 G_CALLBACK(delete_event), rdata);
	g_signal_connect(rdata->rdeve_gui, "destroy",
			 G_CALLBACK(destroy), rdata);
	...
	//
	gtk_container_add(GTK_CONTAINER(rdata->rdeve_gui),
			  rdata->rdeve_box1);
	...
	gtk_box_pack_start(GTK_BOX(rdata->rdeve_box2), label, FALSE, FALSE,
			   0);
	gtk_box_pack_start(GTK_BOX(rdata->rdeve_box2),
			   rdata->rdeve_devid_entry, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rdeve_box2),
			   rdata->rdeve_devid_load, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rdeve_box1), rdata->rdeve_box2,
			   FALSE, FALSE, 0);



	rdata->rdeve_text_editor = gtk_text_view_new();
	rdata->rdeve_text_buffer =
	    gtk_text_view_get_buffer(GTK_TEXT_VIEW
				     (rdata->rdeve_text_editor));
	rdata->rdeve_status_bar = gtk_statusbar_new();
	rdata->rdeve_text_editor_vscroll =
	    gtk_vscrollbar_new(gtk_text_view_get_vadjustment
			       (GTK_TEXT_VIEW(rdata->rdeve_text_editor)));
	rdata->rdeve_text_editor_hscroll =
	    gtk_hscrollbar_new(gtk_text_view_get_hadjustment
			       (GTK_TEXT_VIEW(rdata->rdeve_text_editor)));
	gtk_widget_set_size_request(rdata->rdeve_text_editor, 600, 600);
	gtk_text_buffer_set_text(rdata->rdeve_text_buffer, "", 0);

	gtk_box_pack_start(GTK_BOX(rdata->rdeve_box4),
			   rdata->rdeve_text_editor, TRUE, TRUE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rdeve_box4),
			   rdata->rdeve_text_editor_vscroll, FALSE, FALSE,
			   0);
	gtk_box_pack_start(GTK_BOX(rdata->rdeve_box1), rdata->rdeve_box4,
			   FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rdeve_box1),
			   rdata->rdeve_text_editor_hscroll, FALSE, FALSE,
			   0);

	g_signal_connect(rdata->rdeve_devid_load, "clicked",
			 G_CALLBACK(rdeve_load_devid), rdata);
	g_signal_connect(rdata->rdeve_text_buffer, "changed",
			 G_CALLBACK(rdeve_update_statusbar), rdata);
	g_signal_connect(rdata->rdeve_text_buffer, "mark_set",
			 G_CALLBACK(rdeve_mark_set_callback), rdata);

	gtk_widget_grab_focus(rdata->rdeve_text_editor);
	rdata->rdeve_config_upload =
	    gtk_button_new_with_label("Upload config");
	gtk_box_pack_start(GTK_BOX(rdata->rdeve_box3),
			   rdata->rdeve_config_upload, FALSE, FALSE, 0);
	rdata->rdeve_config_check =
	    gtk_button_new_with_label("Check config");
	gtk_box_pack_start(GTK_BOX(rdata->rdeve_box3),
			   rdata->rdeve_config_check, FALSE, FALSE, 0);
	rdata->rdeve_event_edit = gtk_button_new_with_label("Edit events");
	gtk_box_pack_start(GTK_BOX(rdata->rdeve_box3),
			   rdata->rdeve_event_edit, FALSE, FALSE, 0);
	...
	g_signal_connect(rdata->rdeve_event_edit, "clicked",
			 G_CALLBACK(rdeve_event_edit), rdata);

	gtk_box_pack_start(GTK_BOX(rdata->rdeve_box1), rdata->rdeve_box3,
			   FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rdeve_box1),
			   rdata->rdeve_status_bar, FALSE, FALSE, 0);

	gtk_widget_show_all(rdata->rdeve_gui);
}


void rserver_settings(GtkWidget * some, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	rdata->rserv_gui = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_position(GTK_WINDOW(rdata->rserv_gui),
				GTK_WIN_POS_CENTER);
	gtk_window_set_resizable(GTK_WINDOW(rdata->rserv_gui), FALSE);
	g_signal_connect(rdata->rserv_gui, "delete-event",
			 G_CALLBACK(rserver_settings_delete), rdata);
	g_signal_connect(rdata->rserv_gui, "destroy",
			 G_CALLBACK(rserver_settings_destroy), rdata);
	...
	//
	rdata->rserv_users_update =
	    gtk_button_new_with_label("Update users");
	...
	gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW
						    ...
						    "text", COL_AGE, NULL);
	//
	g_signal_connect(rdata->rserv_users_box, "row-activated",
			 G_CALLBACK(rserver_settings_users_box), rdata);

	rdata->rserv_adduser_button =
	    gtk_button_new_with_label("Add user");
	g_signal_connect(rdata->rserv_adduser_button, "clicked",
			 G_CALLBACK(rserver_settings_adduser), rdata);
	gtk_container_add(GTK_CONTAINER(rdata->rserv_gui),
			  rdata->rserv_box5);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box5), rdata->rserv_box1,
			   FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box0),
			   rdata->rserv_users_update, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box0),
			   rdata->rserv_users_box, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box0),
			   rdata->rserv_adduser_button, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box1), rdata->rserv_box0,
			   FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box1), rdata->rserv_box2,
			   FALSE, FALSE, 0);
	...
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box2),
			   rdata->rserv_box_moduser_user, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box2),
			   rdata->rserv_box_user, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box2),
			   rdata->rserv_box_pass, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box2),
			   rdata->rserv_box_pass_c, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box2),
			   rdata->rserv_box_is_admin, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box2),
			   rdata->rserv_box_button, FALSE, FALSE, 0);
	//
	rdata->rserv_entry_moduser_user = gtk_entry_new();
	rdata->rserv_label_moduser_user = gtk_label_new("moduser user:\t");
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box_moduser_user),
			   rdata->rserv_label_moduser_user, FALSE, FALSE,
			   0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box_moduser_user),
			   rdata->rserv_entry_moduser_user, FALSE, FALSE,
			   0);
	//
	rdata->rserv_entry_user = gtk_entry_new();
	rdata->rserv_label_user = gtk_label_new("new username:\t");
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box_user),
			   rdata->rserv_label_user, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box_user),
			   rdata->rserv_entry_user, FALSE, FALSE, 0);
	//
	rdata->rserv_entry_pass = gtk_entry_new();
	rdata->rserv_label_pass = gtk_label_new("new password:\t");
	gtk_entry_set_visibility(GTK_ENTRY(rdata->rserv_entry_pass),
				 FALSE);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box_pass),
			   rdata->rserv_label_pass, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box_pass),
			   rdata->rserv_entry_pass, FALSE, FALSE, 0);
	//
	rdata->rserv_entry_pass_c = gtk_entry_new();
	rdata->rserv_label_pass_c = gtk_label_new("confirm pass:\t\t");
	gtk_entry_set_visibility(GTK_ENTRY(rdata->rserv_entry_pass_c),
				 FALSE);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box_pass_c),
			   rdata->rserv_label_pass_c, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box_pass_c),
			   rdata->rserv_entry_pass_c, FALSE, FALSE, 0);
	//
	rdata->rserv_is_admin = gtk_check_button_new();
	rdata->rserv_label_is_admin = gtk_label_new("is admin:\t\t\t");
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box_is_admin),
			   rdata->rserv_label_is_admin, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box_is_admin),
			   rdata->rserv_is_admin, FALSE, FALSE, 0);
	//
	rdata->rserv_deluser_button =
	    gtk_button_new_with_label("Del user");
	rdata->rserv_moduser_button =
	    gtk_button_new_with_label("Mod user");
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box_button),
			   rdata->rserv_deluser_button, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box_button),
			   rdata->rserv_moduser_button, FALSE, FALSE, 0);
	g_signal_connect(rdata->rserv_deluser_button, "clicked",
			 G_CALLBACK(rserver_settings_deluser), rdata);
	g_signal_connect(rdata->rserv_moduser_button, "clicked",
			 G_CALLBACK(rserver_settings_moduser), rdata);
	//
	rdata->rserv_separator2 = gtk_hseparator_new();
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box5),
			   rdata->rserv_separator2, FALSE, FALSE, 0);
	//
	rdata->rserv_entry_your_pass = gtk_entry_new();
	rdata->rserv_label_your_pass = gtk_label_new("your pass:\t\t");
	gtk_entry_set_visibility(GTK_ENTRY(rdata->rserv_entry_your_pass),
				 FALSE);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box_your_pass),
			   rdata->rserv_label_your_pass, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box_your_pass),
			   rdata->rserv_entry_your_pass, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box5),
			   rdata->rserv_box_your_pass, FALSE, FALSE, 0);
	//
	rdata->rserv_separator3 = gtk_hseparator_new();
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box5),
			   rdata->rserv_separator3, FALSE, FALSE, 0);
	//
	rdata->rserv_status_label =
	    gtk_label_new
	    ("Your pass is cleared within 120sec of inactivity");
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box5),
			   rdata->rserv_status_label, FALSE, FALSE, 0);
	//
	// Set tty part: 
	rdata->rserv_separator1 = gtk_vseparator_new();
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box1),
			   rdata->rserv_separator1, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box1), rdata->rserv_box3,
			   FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box1), rdata->rserv_box4,
			   FALSE, FALSE, 0);
	...
	g_signal_connect(rdata->rserv_users_box, "row-activated",
			 G_CALLBACK(rserver_settings_users_box), rdata);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box3),
			   rdata->rserv_tty_update, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box3),
			   rdata->rserv_tty_box, FALSE, FALSE, 0);
	...
	gtk_box_pack_start(GTK_BOX(rdata->rserv_tty_entry_box),
			   rdata->rserv_tty_label, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_tty_entry_box),
			   rdata->rserv_tty_entry, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box4),
			   rdata->rserv_tty_entry_box, FALSE, FALSE, 0);
	rdata->rserv_tty_list_units =
	    gtk_button_new_with_label("List pattern");
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box4),
			   rdata->rserv_tty_list_units, FALSE, FALSE, 0);
	g_signal_connect(rdata->rserv_tty_list_units, "clicked",
			 G_CALLBACK(rserver_settings_tty_list), rdata);
	rdata->rserv_tty_open =
	    gtk_button_new_with_label("Open pattern and write to config");
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box4),
			   rdata->rserv_tty_open, FALSE, FALSE, 0);
	g_signal_connect(rdata->rserv_tty_open, "clicked",
			 G_CALLBACK(rserver_settings_tty_open), rdata);
	rdata->rserv_tty_close =
	    gtk_button_new_with_label("Close current tty");
	gtk_box_pack_start(GTK_BOX(rdata->rserv_box4),
			   rdata->rserv_tty_close, FALSE, FALSE, 0);
	g_signal_connect(rdata->rserv_tty_close, "clicked",
			 G_CALLBACK(rserver_settings_tty_close), rdata);
	//
	gtk_widget_show_all(rdata->rserv_gui);

	rdata->rserv_your_clear_timeout =
	    g_timeout_add(1000, rserver_settings_your_clear, rdata);
	rdata->rserv_your_clear_counter = 0;
...
void rdev_gui(GtkWidget * some, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	...
	gtk_window_set_position(GTK_WINDOW(rdata->rdev_gui),
				GTK_WIN_POS_CENTER);
	gtk_window_set_resizable(GTK_WINDOW(rdata->rdev_gui), FALSE);
	g_signal_connect(rdata->rdev_gui, "delete-event",
			 G_CALLBACK(rdev_gui_delete), rdata);
	g_signal_connect(rdata->rdev_gui, "destroy",
			 G_CALLBACK(rdev_gui_destroy), rdata);

	rdata->rdev_box2 = gtk_vbox_new(FALSE, 10);
	rdata->rdev_box1 = gtk_hbox_new(FALSE, 10);
	gtk_container_add(GTK_CONTAINER(rdata->rdev_gui),
			  rdata->rdev_box2);
	gtk_box_pack_start(GTK_BOX(rdata->rdev_box2), rdata->rdev_box1,
			   FALSE, FALSE, 0);
	rdata->rdev_box1_addr1 = gtk_vbox_new(FALSE, 10);
	rdata->rdev_box1_addr2 = gtk_vbox_new(FALSE, 10);
	gtk_box_pack_start(GTK_BOX(rdata->rdev_box1),
			   rdata->rdev_box1_addr1, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rdev_box1),
			   rdata->rdev_box1_addr2, FALSE, FALSE, 0);
	...
	g_signal_connect(rdata->rdev_stamp_button, "clicked",
			 G_CALLBACK(rdev_stamp), rdata);
	if (rdata->config_byte & 0x00000010)
		rdata->rdev_stamp_label =
		    gtk_label_new("Adress is stamped");
	else
		rdata->rdev_stamp_label =
		    gtk_label_new("Adress not stamped");

	gtk_box_pack_start(GTK_BOX(rdata->rdev_box1_addr1),
			   rdata->rdev_label_addr1, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rdev_box1_addr1),
			   rdata->rdev_addr_tbox1, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rdev_box1_addr2),
			   rdata->rdev_label_addr2, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rdev_box1_addr2),
			   rdata->rdev_addr_tbox2, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rdev_box1),
			   rdata->rdev_stamp_button, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rdev_box2),
			   rdata->rdev_stamp_label, FALSE, FALSE, 0);
	char tmp[30];
...
void rspeed_set_debug(GtkWidget * some, gpointer data)
{
	rspeed_gui_rep *rdata = (rspeed_gui_rep *) data;
	if (debug) {
...
void rnotify_show(GtkWidget * some, gpointer data)
{
	...
	gtk_window_set_position(GTK_WINDOW(rdata->rnotify_gui),
				GTK_WIN_POS_CENTER);
	gtk_window_set_resizable(GTK_WINDOW(rdata->rnotify_gui), FALSE);
	gtk_window_set_title(GTK_WINDOW(rdata->rnotify_gui),
			     "Notification list");
	...
	rdata->rnotify_list_list =
	    gtk_list_store_new(4, G_TYPE_STRING, G_TYPE_UINT, G_TYPE_UINT,
			       G_TYPE_STRING);
	renderer = gtk_cell_renderer_text_new();
	gtk_tree_view_insert_column_with_attributes(GTK_TREE_VIEW
						    (rdata->rnotify_list),
	...
	gtk_container_add(GTK_CONTAINER(rdata->rnotify_gui),
			  rdata->rnotify_box1);
	gtk_box_pack_start(GTK_BOX(rdata->rnotify_box1),
			   rdata->rnotify_list, FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->rnotify_box1),
			   rdata->rnotify_label, FALSE, FALSE, 0);
...
void rspeed_gui(gpointer * data)
{
	...
	gtk_window_set_position(GTK_WINDOW(rdata->window),
				GTK_WIN_POS_CENTER);
	gtk_window_set_resizable(GTK_WINDOW(rdata->window), FALSE);
	g_signal_connect(rdata->window, "delete-event",
			 G_CALLBACK(delete_event), &rdata->open);
	g_signal_connect(rdata->window, "destroy",
			 G_CALLBACK(destroy), &rdata->open);
	...
	if (rdata->is_admin && rdata->remote)
		rdata->rdev_dev_edit_button =
		    gtk_button_new_with_label("Dev editor");
	if (rdata->is_admin && rdata->remote)
		rdata->rdev_is_admin_button =
		    gtk_button_new_with_label("Settings");
	rdata->rdev_show_notify =
	    gtk_button_new_with_label("Notifications");
	gtk_container_add(GTK_CONTAINER(rdata->window), rdata->box2);
	gtk_box_pack_start(GTK_BOX(rdata->box2), rdata->box1, FALSE, FALSE,
			   0);
	gtk_box_pack_start(GTK_BOX(rdata->box2), rdata->separator1, FALSE,
			   TRUE, 5);
	gtk_box_pack_start(GTK_BOX(rdata->box1), rdata->scan_button, FALSE,
			   FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->box1), rdata->pbar, FALSE, FALSE,
			   0);
	gtk_box_pack_start(GTK_BOX(rdata->box1), rdata->scan_list, FALSE,
			   FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->box4), rdata->debug_button,
			   FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->box4), rdata->devbutton, TRUE,
			   TRUE, 0);
	if (rdata->is_admin && rdata->remote)
		gtk_box_pack_start(GTK_BOX(rdata->box4),
				   rdata->rdev_dev_edit_button, TRUE, TRUE,
				   0);
	gtk_box_pack_start(GTK_BOX(rdata->box1), rdata->box4, FALSE, FALSE,
			   0);
	if (rdata->is_admin && rdata->remote)
		gtk_box_pack_start(GTK_BOX(rdata->box5),
				   rdata->rdev_is_admin_button, FALSE,
				   FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->box5), rdata->rdev_show_notify,
			   FALSE, FALSE, 0);
	gtk_box_pack_start(GTK_BOX(rdata->box1), rdata->box5, FALSE, FALSE,
			   0);
	gtk_box_pack_start(GTK_BOX(rdata->box1), rdata->label, FALSE,
			   FALSE, 0);
	// Signals
	g_signal_connect(rdata->scan_list, "row-activated",
			 G_CALLBACK(view_onRowActivated), rdata);
	g_signal_connect(rdata->debug_button, "clicked",
			 G_CALLBACK(rspeed_set_debug), rdata);
	g_signal_connect(rdata->devbutton, "clicked", G_CALLBACK(rdev_gui),
			 rdata);
	if (rdata->is_admin && rdata->remote)
		g_signal_connect(rdata->rdev_dev_edit_button, "clicked",
				 G_CALLBACK(rdev_editor), rdata);
	if (rdata->is_admin && rdata->remote)
		g_signal_connect(rdata->rdev_is_admin_button, "clicked",
				 G_CALLBACK(rserver_settings), rdata);
	g_signal_connect(rdata->rdev_show_notify, "clicked",
			 G_CALLBACK(rnotify_show), rdata);
	//
	...
	if (pdata->remote) {
		pthread_create(&rdata->printr, NULL, &client_handler,
			       (void *) rdata);
	} else {
		pthread_create(&rdata->printr, NULL, &print_ser_gui,
			       (void *) rdata);
	}
...
static void trayConnect(GtkMenuItem * item, gpointer data)
{
	ProgressData *pdata = (ProgressData *) data;
	pdata->open = 1;
...
static void trayMainUI(GtkMenuItem * item, gpointer data)
{
	ProgressData *pdata = (ProgressData *) data;
	if (pdata->share != NULL) {
...
bool show_ok(const gchar * msg, const gchar * title, gpointer data,
	     GtkMessageType type)
{
	ProgressData *pdata = (ProgressData *) data;
	GtkWidget *dialog;
	dialog = gtk_message_dialog_new(GTK_WINDOW(pdata->window),
...
bool show_yes_no(const char *msg, const gchar * title, gpointer data,
		 GtkMessageType type)
{
	ProgressData *pdata = (ProgressData *) data;
	GtkWidget *dialog;
...
static bool set_autoconnect(GtkWidget * button, gpointer data)
{
	...
	if (gtk_toggle_button_get_active
	    (GTK_TOGGLE_BUTTON(pdata->auto_connect))) {
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON
					     (pdata->save_login), TRUE);
	...
	if (!auto_login)
		auto_login =
		    config_setting_add(config_root_setting(&cfg),
				       "auto_login", CONFIG_TYPE_BOOL);
	if (!password)
		password =
		    config_setting_add(config_root_setting(&cfg),
				       "password", CONFIG_TYPE_STRING);

	config_setting_set_bool(auto_login,
...
static bool set_save_login(GtkWidget * button, gpointer data)
{
	...
	if (!host)
		host =
		    config_setting_add(config_root_setting(&cfg),
				       "server_host", CONFIG_TYPE_STRING);
	if (!username)
		username =
		    config_setting_add(config_root_setting(&cfg),
				       "username", CONFIG_TYPE_STRING);
	if (gtk_toggle_button_get_active
	    (GTK_TOGGLE_BUTTON(pdata->save_login))) {
...
static bool connect_to_server(GtkWidget * button, gpointer data)
{
	...
	if (pdata->connected) {
		if (pdata->share != NULL)
			pthread_cancel(((rspeed_gui_rep *) pdata->share)->
				       printr);
	...
	if (pdata->save_con) {
		...
		if (!config_read_file(&pdata->main_cfg, "main.cfg")) {
			...
			if (fgets(tmp, 50, pipe) == NULL) {
				...
				printf("Line %d: %s\n",
				       config_error_line(&pdata->main_cfg),
				       config_error_text(&pdata->
							 main_cfg));
			}
	...
	if (sig < 1) {
		if (sig == -1) {
			if (show_yes_no
				...
				if (show_yes_no
					...
					if (!pdata->sslc.
					    add_known_hosts(host_entry)) {
						show_ok
						    ("Unable to write to file, known_hosts",
						     "Info", pdata,
						     GTK_MESSAGE_WARNING);
					}
				}
		...
		if (sig == 0) {
			if (show_yes_no
			    ("The host where not recognized, add to known hosts? (yes/no): ",
			     "Question", pdata, GTK_MESSAGE_QUESTION)) {
				if (!pdata->sslc.
				    add_known_hosts(host_entry)) {
					show_ok
					    ("Unable to write to file, known_hosts",
		...
		show_ok
		    ("Found the CA fingerprint in known_host, addning new record with this IP",
		     "Info", pdata, GTK_MESSAGE_INFO);
	...
	pdata->remote = 1;
	if (pdata->share == NULL
	    || ((rspeed_gui_rep *) pdata->share)->open != 1) {
		rspeed_gui((gpointer *) pdata);
	} else {
		if (pdata->is_admin !=
		    ((rspeed_gui_rep *) pdata->share)->is_admin) {
			pthread_cancel(((rspeed_gui_rep *) pdata->share)->
				       printr);
			gtk_widget_destroy(GTK_WIDGET
					   (((rspeed_gui_rep *) pdata->
					     share)->window));
			rspeed_gui((gpointer *) pdata);
		} else {
			pthread_create(&((rspeed_gui_rep *) pdata->share)->
				       printr, NULL, &client_handler,
				       pdata->share);
		}
...
int main(int argc, char *argv[])
{
	...
	if (!config_read_file(&pdata->main_cfg, "main.cfg")) {
		...
		if (fgets(tmp, 50, pipe) == NULL) {
			...
			printf("Line %d: %s\n",
			       config_error_line(&pdata->main_cfg),
			       config_error_text(&pdata->main_cfg));
		}
		config_destroy(&pdata->main_cfg);
	...
	gtk_window_set_title(GTK_WINDOW(pdata->window),
			     "Select Spb interface");

	pdata->trayIcon = gtk_status_icon_new_from_file("spb.png");
	//set popup menu for tray icon
	pdata->menu = gtk_menu_new();
	pdata->menuItemConnect =
	    gtk_menu_item_new_with_label("Set Interface");
	pdata->menuItemMainUI = gtk_menu_item_new_with_label("Spb UI");
	pdata->menuItemExit = gtk_menu_item_new_with_label("Exit");
	g_signal_connect(G_OBJECT(pdata->menuItemConnect), "activate",
			 G_CALLBACK(trayConnect), pdata);
	g_signal_connect(G_OBJECT(pdata->menuItemMainUI), "activate",
			 G_CALLBACK(trayMainUI), pdata);
	g_signal_connect(G_OBJECT(pdata->menuItemExit), "activate",
			 G_CALLBACK(trayExit), pdata);
	gtk_menu_shell_append(GTK_MENU_SHELL(pdata->menu),
			      pdata->menuItemConnect);
	gtk_menu_shell_append(GTK_MENU_SHELL(pdata->menu),
			      pdata->menuItemMainUI);
	gtk_menu_shell_append(GTK_MENU_SHELL(pdata->menu),
			      pdata->menuItemExit);
	...
	g_signal_connect(GTK_STATUS_ICON(pdata->trayIcon), "activate",
			 GTK_SIGNAL_FUNC(trayIconActivated), pdata);
	g_signal_connect(GTK_STATUS_ICON(pdata->trayIcon), "popup-menu",
			 GTK_SIGNAL_FUNC(trayIconPopup), pdata->menu);
	...
	gtk_menu_shell_append(GTK_MENU_SHELL(pdata->menuBar),
			      pdata->menuItemTopLvl);

	pdata->mainMenu = gtk_menu_new();
	gtk_menu_item_set_submenu(GTK_MENU_ITEM(pdata->menuItemTopLvl),
				  pdata->mainMenu);
	pdata->mainMenuItemExit = gtk_menu_item_new_with_label("Quit");
	g_signal_connect(G_OBJECT(pdata->mainMenuItemExit), "activate",
			 G_CALLBACK(trayExit), NULL);
	gtk_menu_shell_append(GTK_MENU_SHELL(pdata->mainMenu),
			      pdata->mainMenuItemExit);
	...
	g_signal_connect(pdata->window, "delete-event",
			 G_CALLBACK(delete_event), &pdata->open);

	/* Here we connect the "destroy" event to a signal handler.  
	 * This event occurs when we call gtk_widget_destroy() on the window,
	 * or if we return FALSE in the "delete-event" callback. */
	g_signal_connect(pdata->window, "destroy",
			 G_CALLBACK(destroy), &pdata->open);
	...
	g_signal_connect(button1, "clicked",
			 G_CALLBACK(open_tty_button), pdata);
	...
	pdata->auto_connect =
	    gtk_check_button_new_with_label("Save pass(auto connect)");
	pdata->save_login =
	    gtk_check_button_new_with_label("Save user and host");
	//
	...
	gtk_box_pack_start(GTK_BOX(box4), pdata->server_label, FALSE,
			   FALSE, 0);
	gtk_box_pack_start(GTK_BOX(box4), pdata->server_adress, FALSE,
			   FALSE, 0);

	const char *str_tmp;
	int alogin = 0;
	if (config_lookup_string
	    (&pdata->main_cfg, "server_host", &str_tmp)) {
		gtk_entry_set_text(GTK_ENTRY(pdata->server_adress),
				   str_tmp);
		alogin++;
		if (strlen(str_tmp) > 0)
			gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON
	...
	gtk_box_pack_start(GTK_BOX(box2), pdata->label_name, FALSE, FALSE,
			   0);
	gtk_box_pack_start(GTK_BOX(box2), pdata->login_name, FALSE, FALSE,
			   0);
	...
	gtk_box_pack_start(GTK_BOX(box3), pdata->label_pwd, FALSE, FALSE,
			   0);
	gtk_box_pack_start(GTK_BOX(box3), pdata->login_pwd, FALSE, FALSE,
			   0);
	...
	if (config_lookup_bool
	    (&pdata->main_cfg, "auto_login", &auto_login)) {
		if (auto_login)
			gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON
						     (pdata->auto_connect),
						     TRUE);
	...
	pdata->connect_button =
	    gtk_button_new_with_label("Connect to server");

	g_signal_connect(pdata->connect_button, "clicked",
			 G_CALLBACK(connect_to_server), pdata);
	...
	gtk_box_pack_start(GTK_BOX(box1), pdata->save_login, FALSE, FALSE,
			   0);
	gtk_box_pack_start(GTK_BOX(box1), pdata->auto_connect, FALSE,
			   FALSE, 0);
	gtk_box_pack_start(GTK_BOX(box1), pdata->connect_button, FALSE,
			   FALSE, 0);
