...
void exec_package(void *ptr, char *data, int counter){
  ...
  if(debug){
    ...
    case 0:
      std::cerr << "Recived broadcast aknowlegde (0x00) from '" << std::dec  << (data[2] & 0xff) << "." <<
	(data[3] & 0xff) << "' to '" << std::dec  << (data[0] & 0xff) << "." << (data[1] & 0xff) << "'";
      break;
    case 1:
	...
	(data[3] & 0xff) << "'";
      break;
    case 3:
      std::cerr << "Recived config fetch (0x03) from '" << std::dec  << (data[2] & 0xff) << "." <<
	(data[3] & 0xff) << "'";
      break;
    case 170:
      wtime();
...
void *client_handler(void *ptr){
  ...
  while(1){
	...
	gtk_list_store_set (rdata->rserv_store_tty, &rdata->rserv_iter,
			    COL_NAME, tty,
			    -1);
...
static void tty_to_combo(GtkWidget *combo){
  ...
  int lwen;
  char *tty;
  if(pipe == NULL){g_print("Faild to open");exit(1);}
  while(lwen = getline(&tty,&len,pipe) > 0){
    glist = g_list_append (glist, (gpointer*)tty);tty=NULL;}
  pclose(pipe);
...
static bool open_tty_button( GtkWidget *widget,
                   gpointer   data )
{
  addr1 = 20;
  addr2 = 20;
...
static int get_variable_gui(void *data, int var_id, bool from_gui){
  ...
  if(strcmp(rdata->spb_widget_variable_type[var_id],"scale") == 0){
    return (char)gtk_adjustment_get_value(gtk_range_get_adjustment(GTK_RANGE(rdata->spb_widgets[rdata->spb_widget_variable_source[var_id]])));
  }
  }else{
...
static char set_variable_gui(void *data, int var_id, int v_data){
  ...
  if(strcmp(rdata->spb_widget_variable_type[var_id],"label") == 0){
    sprintf(print,rdata->spb_widget_vars_printf[widget_id],get_variable_gui(rdata,rdata->spb_widget_vars[widget_id][1]+257,0),
	    get_variable_gui(rdata,rdata->spb_widget_vars[widget_id][2]+257,0),get_variable_gui(rdata,rdata->spb_widget_vars[widget_id][3]+257,0));
	...
	if((rdata->backe->event_exist[ii]) && strcmp(rdata->backe->event_exec[i][ii],"getvars") == 0){
	  ...
	  for(int iii=0; iii <count; iii++){
		...
		for(int iiii=0; iiii <count; iiii++){
		  set_variable_gui(rdata,rdata->backe->event_data1[i][ii][iiii+1],(unsigned char)p_data[rdata->backe->event_data2[i][ii][iiii+1]+6]);
		}
...
void view_onRowActivated (GtkTreeView *treeview,GtkTreePath *path,GtkTreeViewColumn *col,gpointer data){
   rspeed_gui_rep *rdata = (rspeed_gui_rep *)data;
   GtkTreeModel *model;
   GtkTreeIter   iter;
...
gboolean rdev_stamp(GtkWidget *some,gpointer data){
  ...
  if((getaddr1 == 0 && getcaddr1[0] != '0') || (getaddr2 == 0 && getcaddr2[0] != '0') || getaddr1 > 255 || getaddr2 < 0 || getaddr2 > 255 || getaddr2 < 0){
  ...
  //std::cout << "Address: " << getaddr1 << "." << getaddr2 << "\n";
	...
	sprintf(tmp,"No response on stamp...",getaddr1,getaddr2);
      ...
      int len = 11;
      char getdevs[50] = {rdata->addr1,rdata->addr2,addr1,addr2,0x03,0x00,0x03,0x01,getaddr1,getaddr2,0x00};
      m_send(rdata, getdevs, len);
      usleep(500000);
      if(got_resp){
      ...
      }else
	{
	  rdata->got_rec = 0;
	  int len2 = 7;
          char getdevs2[50] = {getaddr1,getaddr2,addr1,addr2,0x03,0x00,0x01,0x00};
...
void rserver_settings_users_box(GtkTreeView *treeview,GtkTreePath *path,GtkTreeViewColumn *col,gpointer data){
  rspeed_gui_rep *rdata = (rspeed_gui_rep *)data;
  GtkTreeModel *model;
  GtkTreeIter   iter;
  model = gtk_tree_view_get_model(treeview);
...
gboolean rserver_settings_delete(GtkWidget *some,GdkEvent  *event,gpointer data){
  rspeed_gui_rep *rdata = (rspeed_gui_rep *)data; 
  g_source_remove (rdata->rserv_your_clear_timeout);
  gtk_widget_destroy (some);
  return 1;
...
bool rdeve_fetch_config(gpointer data){
  ...
  char *ctext = gtk_text_buffer_get_text (rdata->rdeve_text_buffer, &start, &end, FALSE);
...
gboolean rdeve_event_change(GtkWidget *buffer,
			  gpointer data){
  rspeed_gui_rep *rdata = (rspeed_gui_rep *)data;
...
gboolean rdeve_event_save(GtkWidget *buffer,
			    gpointer data){
  rspeed_gui_rep *rdata = (rspeed_gui_rep *)data;
...
gboolean rdeve_event_load(GtkWidget *buffer,
			    gpointer data){
  rspeed_gui_rep *rdata = (rspeed_gui_rep *)data;
			...
			if(bits_c == at_c && at_c == data_c){
			  for(int i = 0; i < data_c; ++i)
				...
				if(!rdata->rdevee_load_type_hardcode){
				  ...
				  g_signal_connect(G_OBJECT(rdata->rdevee_combo_opt), "changed", 
						   G_CALLBACK(rdeve_event_change), rdata);
				...
				if(opts_t != NULL)
				  {
					...
					{
					  if(oi > MAX_WIDGETS){
					    gtk_statusbar_push(GTK_STATUSBAR(rdata->rdeve_status_bar), 0, "The number of options exceeded MAX_WIDGETS, please extend the value and recompile, if this is a problem");
					    break;
					  }
					  config_setting_t *opts_e = config_setting_get_elem(opts_t, oi);
					  const char *oname;
					  ...
					  if(oobits > 1){
					    ...
					    if(oevalue)
					      gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(rdata->rdevee_elem_input[oi]),1);
					  }
					    rdata->rdevee_elem_label[oi] = gtk_label_new(oname);
					    gtk_box_pack_start (GTK_BOX (rdata->rdevee_elem_box[oi]), rdata->rdevee_elem_input[oi], FALSE, FALSE, 0);
					    gtk_box_pack_start (GTK_BOX (rdata->rdevee_elem_box[oi]), rdata->rdevee_elem_label[oi], FALSE, FALSE, 0);
					    gtk_box_pack_start (GTK_BOX (rdata->rdevee_ebox), rdata->rdevee_elem_box[oi], FALSE, FALSE, 0);
			  ...
			  gtk_statusbar_push(GTK_STATUSBAR(rdata->rdeve_status_bar), 0, "data, at, bits do not have the same number of elements");
			}
		      }
		  }
	      }
...
gboolean rdeve_event_edit(GtkWidget *buffer,
			    gpointer data){
  rspeed_gui_rep *rdata = (rspeed_gui_rep *)data;
...
void rdev_gui(GtkWidget *some,gpointer data){
  ...
  rdata->got_rec = 0;
    ...
    int len = 8;
    char getdevs[20] = {rdata->addr1,rdata->addr2,addr1,addr2,0x03,0x00,0x03,0x00};
    m_send(rdata, getdevs, len);  
    usleep(500000);
    if(rdata->got_rec == 3)
    	break;
...
bool show_ok(const gchar *msg, const gchar *title,gpointer data, GtkMessageType type){
  ProgressData *pdata = (ProgressData *)data;
  GtkWidget *dialog;
...
bool show_yes_no(const char *msg, const gchar *title,gpointer data, GtkMessageType type)
{
  ProgressData *pdata = (ProgressData *)data;
  GtkWidget *dialog;
  dialog = gtk_message_dialog_new(GTK_WINDOW(pdata->window),
...
static bool connect_to_server(GtkWidget *button, gpointer data){
  ...
  if(sig < 1){
    if(sig == -1){
      if(show_yes_no("POTENSIAL MIME ATTACK!!!!\nDANGERUS!!! Do NOT proced if your not werry sure that the certificate has been replaced!!!\nDo you want to proceed?", "Alert!", pdata, GTK_MESSAGE_ERROR)){
      if(show_yes_no("Add to known hosts? (yes/no): ","Question",pdata,GTK_MESSAGE_QUESTION)){if(!pdata->sslc.add_known_hosts(host_entry)){show_ok("Unable to write to file, known_hosts","Info",pdata,GTK_MESSAGE_WARNING);}}
}else{gtk_label_set_text(GTK_LABEL(pdata->label1),"Connection Cancled");pdata->sslc.sslfree();return 0;}}
    if(sig == 0){
      if(show_yes_no("The host where not recognized, add to known hosts? (yes/no): ","Question",pdata,GTK_MESSAGE_QUESTION)){if(!pdata->sslc.add_known_hosts(host_entry)){show_ok("Unable to write to file, known_hosts","Info",pdata,GTK_MESSAGE_WARNING);}}}
  }else if(sig == 2){
