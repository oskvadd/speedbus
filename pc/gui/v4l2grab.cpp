...
static char deviceName[] = "/dev/video3";
...
*/
static void jpegWrite(unsigned char* img, print_seri * serial_p, int listnum)
{
  ...
  int i = 0;
  char cbuf[4050];
  while (i <= (outsize/4000))
    {
      serial_p->surv_resp = 0;

      strcpy(cbuf, "camei ");
      int cpysize;
      if(((i*4000)+4000) >= outsize)
	cpysize = outsize - (i*4000); 
      else
	cpysize = 4000;
      memcpy(&cbuf[6], &outbuffer[i*4000], cpysize);
      serial_p->server->send_data(listnum, cbuf, cpysize + 6);
...
/**
  process image read
*/
static void imageProcess(const void* p, print_seri * serial_p, int listnum)
{
  ...
  // write jpeg
  jpegWrite(dst, serial_p, listnum);
  free(dst);
}
...
static int frameRead(print_seri * serial_p, int listnum)
{
  ...
  switch (io) {
    ...
    case IO_METHOD_READ:
      ...
      imageProcess(buffers[0].start, serial_p, listnum);
      break;
#endif
    ...
    case IO_METHOD_MMAP:
      ...
      imageProcess(buffers[buf.index].start, serial_p, listnum);
    ...
    case IO_METHOD_USERPTR:
      ...
      imageProcess((void *) buf.m.userptr, serial_p, listnum);
...
*/
static void v4l2grab_GetFrame(print_seri * serial_p, int listnum)
{
  unsigned int count;
...
{
  print_seri *print_s = (print_seri *) ptr;
  while(1){
    pthread_mutex_lock(&print_s->surve->mutex);
    pthread_cond_wait(&print_s->surve->cond, &print_s->surve->mutex );
    pthread_mutex_unlock(&print_s->surve->mutex);
    v4l2grab_GetFrame(print_s, print_s->surve->listnum);
