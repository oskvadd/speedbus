...
main_backend *init_backend()
{
	...
	while ((dirp = readdir(dp)) != NULL) {
		...
		int pos =
		    (int) (strstr(dirp->d_name, ".hid") - dirp->d_name);
		if (pos > 0 && pos + 4 == len) {
...
bool backend_set_variable(main_backend * backe, int devid, int var_num,
			  char var_val)
{
...
char backend_get_variable(main_backend * backe, int array_num, int var_num)
{
	if (var_num > 255 && var_num < (255 + MAX_VARIABLE))
		return backend_exec_ops(backe, array_num, var_num,
					backe->
					event_variable[array_num][var_num -
								  257], 0);
	else
		return 0;
}
...
int backend_exec_ops(main_backend * backe, int array_num, int var_num,
		     int var_val, bool from_gui)
{
...
bool backend_exec(main_backend * backe, int event, int devid, short vspace)
{
	...
	for (int i = 0; i < device_num; i++) {
		if (device_id[i] == devid) {	// Device id is found in the MAIN device list
			for (int i = 0; i < backe->devids; i++) {
				if (backe->device_id[i] == devid) {	// Device id is found in the backend device list
					backend_run_event(backe, i, event,
							  vspace);
					return 1;
				}
			}
		       ...
		       short vspace)
{
	...
	if (strcmp(backe->event_exec[array_num][event], "send") == 0) {
		...
		char getdevs[MAX_BUFFER + 7] =
		    { backe->daddr1[array_num], backe->daddr2[array_num],
 addr1, addr2, 0x03, 0x01 };
		for (int ii = 0; ii < len - 7; ii++) {	// Len-7 = rdata->spb_widget_event_data[rdata->spb_widget_event[i]][0] 
			if (backe->event_data[array_num][event][ii + 1] >
			    256) {
				if (backe->
				    event_data[array_num][event][ii + 1] <
				    257 + MAX_VARIABLE) {
...
bool backend_load_var_ops(main_backend * backe, config_t * cfg,
			  char *cname, int opt_devid)
{
...
|| !(config_setting_lookup_int(book, "second_arg", &second_arg))) {
				printf
				    ("You seems to have missed 'variable','device_id','type' or 'second_arg' at event nr %d, in file %s\n",
				     i, cname);
				continue;
			}
...
bool backend_load_events(main_backend * backe)
{
	...
	for (int i = backe->devids; i < device_num; i++) {
		...
		if (!config_read_file(&cfg, tmp)) {
			memset(tmp, 0x00, 50);
			sprintf(tmp, "ls " BACKEND_DIR "devs/|grep %d.spb",
				devid);
		...
		if (setting != NULL) {
			...
			for (int i = 0; i < count; ++i) {
				...
				if (sizeof(event) > MAX_EVENT
					...
					continue;
				strcpy(backe->event_exec[devids][event],
				       type);
