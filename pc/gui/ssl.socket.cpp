...
void sslserver::loadssl()
{
  /*----------------------------------------------------------------*/
	...
	if (SSL_CTX_use_certificate_file
	    (ctx, RSA_SERVER_CERT, SSL_FILETYPE_PEM) <= 0) {
		ERR_print_errors_fp(stderr);
		exit(1);
	...
	if (SSL_CTX_use_PrivateKey_file
	    (ctx, RSA_SERVER_KEY, SSL_FILETYPE_PEM) <= 0) {
		ERR_print_errors_fp(stderr);
		exit(1);
	}
...
bool sslserver::sslsocket()
{
	...
	if (setsockopt
	    (listen_sock, SOL_SOCKET, SO_REUSEADDR, &tr,
	     sizeof(int)) == -1) {
		perror("setsockopt");
		exit(1);
	}
	...
	err =
	    bind(listen_sock, (struct sockaddr *) &sa_serv,
		 sizeof(sa_serv));
	RETURN_ERR(err, "bind");
...
int sslserver::read_data(int listnum)
{
	int errc = 0;
	while (1) {
		timeout.tv_usec = 10000;
		if (select
		    (connectlist[listnum] + 1, &socks, NULL, NULL,
		     &timeout) < 1)
			break;
		err =
		    SSL_read(ssllist[listnum], buf + errc,
			     4096 - (1 + errc));
		errc += err;
		RETURN_SSL(err);
		if (err < 1)
...
bool sslserver::datarun(int listnum, char *data, int *len)
{
...
int sslserver::send_data(int listnum, const char *data, int len)
{
	return SSL_write(ssllist[listnum], data, len);
}

void sslserver::sslfree(int listnum)
{
  /*--------------- SSL closure ---------------*/
	...
	for (listnum = 0; listnum < MAX_LISTEN; listnum++) {
		if (connectlist[listnum] == 0) {
			...
			if (verify_client == ON) {
				...
				client_cert =
				    SSL_get_peer_certificate(ssllist
							     [listnum]);
				if (client_cert != NULL) {
...
bool sslclient::sslsocket(const char *s_ipaddr, short int s_port)
{
/* ------------------------------------------------------------- */
/* Set up a TCP socket */
...
void sslclient::get_ssl_finger(char *finger)
{
...
int sslclient::find_known_hosts(char *host_entry)
{
	FILE *file = fopen("known_hosts", "r");
	if (file != NULL) {
		char line[128];	/* or other suitable maximum line size */
		int s_end =
		    (int) (strchr(host_entry, '|') - host_entry + 1);
		bool mime_flag = 0;
		bool cert_found = 0;

		while (fgets(line, sizeof(line), file) != NULL) {	/* read a line */
			if (strncmp(line, host_entry, strlen(host_entry))
			    == 0) {
				fclose(file);
				return 1;
			}
			if (strncmp(line, host_entry, s_end) == 0)
				mime_flag = 1;
			if (strncmp
			    (line + s_end, host_entry + s_end,
			     strlen(host_entry) - s_end) == 0)
				cert_found = 1;
		}
		fclose(file);
		if (mime_flag)
...
bool sslclient::add_known_hosts(char *host_entry)
{
	FILE *fp;
...
int sslclient::recv_data(char *data)
{
	int err;
...
bool sslclient::send_data(const char *msg, int len)
{
