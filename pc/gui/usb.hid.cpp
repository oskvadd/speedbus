...
{
	dev->devh =
	    libusb_open_device_with_vid_pid(NULL, dev->vid, dev->pid);
...
void *handle_usb_hid_event(void *devv)
{
	...
	while (1) {
		r = libusb_interrupt_transfer(dev->devh, ENDPOINT_INT_IN,
					      answer, dev->bwidth,
					      &transferred, TIMEOUT);
		if (r < 0) {
...
int open_usb_hid(main_backend * backe, char *cname)
{
	...
	if (!config_read_file(&libusb_devices[udevice_num]->cfg, cname)) {
		...
		if (fgets(tmp2, 50, pipe) == NULL) {
			...
			printf("Line %d: %s\n",
			       config_error_line(&libusb_devices
						 [udevice_num]->cfg),
			       config_error_text(&libusb_devices
						 [udevice_num]->cfg));
	...
	if (config_lookup_string
	    (&libusb_devices[udevice_num]->cfg, "vendor_id", &str)) {
		sscanf(str, "%x", &libusb_devices[udevice_num]->vid);
	} else
		return -1;
	if (config_lookup_string
	    (&libusb_devices[udevice_num]->cfg, "product_id", &str)) {
		sscanf(str, "%x", &libusb_devices[udevice_num]->pid);
	} else
		return -1;
	if (!config_lookup_int
	    (&libusb_devices[udevice_num]->cfg, "bwidth",
	     &libusb_devices[udevice_num]->bwidth))
		return -1;
	...
	if (r < 0) {
		fprintf(stderr,
			"Could not find/open LVR Generic HID device\n");
		return 0;
	}
	...
	setting =
	    config_lookup(&libusb_devices[udevice_num]->cfg,
			  "usb_hid.events");
	if (setting != NULL) {
		...
		for (i = 0; i < count; ++i) {
			...
			if (!
			    ...
			    !(config_setting_lookup_int
			      (book, "type", &type))
|| !(config_setting_lookup_int(book, "affected_byte", &affected_byte))
|| !(config_setting_lookup_int(book, "second_arg", &second_arg))
|| !(config_setting_lookup_int(book, "device_id", &devid))) {
				printf
				    ("You seems to have missed 'event','device_id','type','affected_byte' or 'second_arg' at event nr %d in config file %s ignoring event\n",
				     i, cname);
				continue;
			}
			if (affected_byte >
			    libusb_devices[udevice_num]->bwidth) {
				printf
				    ("At event %d in config file %s, affected byte cant be higher then bwidth, row ignored\n",
				     i, cname);
				continue;
			}
			if (type == 0 && second_arg > 7) {	// If there is select bit, in a 8bit value, the range is 0-7
				printf
				    ("At event %d in config file %s, select bit cant be higher then 7, 0-7, row ignored\n",
				     i, cname);
				continue;
			}
		...
		backend_load_var_ops(backe,
				     &libusb_devices[udevice_num]->cfg,
				     cname, 0);
	...
	pthread_create(&libusb_devices[udevice_num]->event_thread, NULL,
		       handle_usb_hid_event,
		       (void *) libusb_devices[udevice_num]);
