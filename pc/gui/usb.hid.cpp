...
void *
handle_usb_hid_event(void *devv)
{
  ...
  while (1)
    {
      r = libusb_interrupt_transfer(dev->devh, ENDPOINT_INT_IN, answer, dev->bwidth, &transferred, TIMEOUT);
      if (r < 0)
	{
...
int
open_usb_hid(main_backend * backe, char *cname)
{
  ...
  if (setting != NULL)
      ...
      for (i = 0; i < count; ++i)
	{
	  ...
	  if (!
	    ...
	    || !(config_setting_lookup_int(book, "second_arg", &second_arg)) || !(config_setting_lookup_int(book, "device_id", &devid)))
	    {
	      printf
	  ...
	  if (type == 0 && second_arg > 7)
	    {			// If there is select bit, in a 8bit value, the range is 0-7
